public class Token {
    ...
    public static final Token orTok = new Token(TokenType.Or, "||"); //define token
    ...
    private Token (TokenType t, String v) {
        ...
        if (t.compareTo(TokenType.Eof) < 0) {
            int ti = t.ordinal();
            reserved[ti] = v;
            token[ti] = this; //keyword save
        }
    } //Token constructor

    public TokenType type( ) { return type; } //return tokentype

    public String value( ) { return value; } //return value

    public static Token keyword  ( String name ) {
        char ch = name.charAt(0);
        if (ch >= 'A' && ch <= 'Z') return mkIdentTok(name);
        for (int i = 0; i < KEYWORDS; i++)
           if (name.equals(reserved[i]))  return token[i]; //if keyword exists, return keyword token
        return mkIdentTok(name);
    } // keyword

    public static Token mkIdentTok (String name) {
        return new Token(TokenType.Identifier, name);
    } //return idenfier token

    public static Token mkIntLiteral (String name) {
        return new Token(TokenType.IntLiteral, name);
    } //return int literal token
