...
public class Parser {
    ...
    private Declarations declarations(Functions fs) {
        ...
        if (isType()) {
            declaration(ds, fs); // save declarations
        }
        return ds;
    }
    ...
    private void declaration(Declarations ds, Functions fs) {
        ...
        if (!token.type().equals(TokenType.Semicolon)) // not global variable
        {
            ds.clear();
            function(fs, v, t);
        } else
    ...
    private Functions functions(Functions fs) {
        while (isType())
            function(fs); // add function to functions
    ...
    private void function(Functions fs) {
        Type t = type();
        Variable v = new Variable(match(TokenType.Identifier)); // create variable
        match(TokenType.LeftParen); // check '('
        Declarations params = params(); // check parameters
        match(TokenType.RightParen); // check ')'
        match(TokenType.LeftBrace); // check '{'
        Declarations locals = declarations(fs); // check local variables
        while (isType()) {
            declaration(locals, fs);
        }
        Block b = statements(v); // check body
        match(TokenType.RightBrace); // check '}'
        Function f = new Function(v, t, params, locals, b);
        fs.members.add(f); // add function to functions
    }
    ...
    private Statement statement(Variable name) {
    ...
    private Block statements(Variable name) {
        // Block --> '{' Statements '}'
        Block b = new Block();
    ...
    private Assignment assignment(Variable name) {
        // Assignment --> Identifier = Expression ;
        Expression source;
        match(TokenType.Assign); // check '='
        source = expression(); // save source
        match(TokenType.Semicolon); // check statement terminator
        return new Assignment(name, source); // return assignment
    }

    private Conditional ifStatement(Variable name) {
        ...
        match(TokenType.RightParen); // check ')'
        s = statement(name); // save statement
        if (token.type().equals(TokenType.Else)) {
            match(TokenType.Else);
            Statement elsestate = statement(name);
    ...
    private Loop whileStatement(Variable name) {
    ...
    private Call callStatement(Variable name) {
        ...
        while (!token.type().equals(TokenType.RightParen)) {
            ...
            p.add(e); // save argument value
            if (token.type().equals(TokenType.Comma))
                match(TokenType.Comma);
        }
        match(TokenType.RightParen);// check ')'
        match(TokenType.Semicolon);// check ';'
        return new Call(name, p);// return call
    }

    private Return returnStatement(Variable name) {
        match(TokenType.Return); // check 'return'
        Expression e = expression(); // calculate expression
        match(TokenType.Semicolon); // check ';'
        return new Return(name, e); // return return
    }
    ...
    private Expression primary() {
        ...
        if (token.type().equals(TokenType.Identifier)) { // check identifier
            Variable v = new Variable(match(TokenType.Identifier));
            if (token.type().equals(TokenType.LeftParen))// check call expression
            {
