...
public class StaticTypeCheck {
    ...
    public static TypeMap typing(Declarations d) {
        ...
        for (Declaration di : d)
            map.put(di.v, di.t); // declaarations에서 변수와 변수 타입을 hash map의 형태로 저장
        return map;
    }
    ...
    public static void V(Program p) {
        V(p.globals); // globals 부분이 올바른지 확인
        System.out.println("");
        V(p.functions, typing(p.globals)); // functions 부분과 type map을 비교하여 올바른지 확인
    ...
    public static Type typeOf(Expression e, TypeMap tm) {
        ...
        if (e instanceof Call) {
            Call c = (Call) e;
            return typeOf(((Call) c).name, tm); // function의 type을 리턴
        }
    ...
    public static void V(Functions fs, TypeMap globals) {
        for (int i = 0; i < fs.members.size(); i++) {
            ...
            for (int j = fs.members.size() - 1; j >= 0; j--) {
                System.out.print("  ");
                System.out.print("<" + fs.members.get(j).name + ", " + fs.members.get(j).type + ", "); // function name,
                                                                                                       // type 출력
                typing(fs.members.get(j).params).display(); // parameter 출력
                if (j != 0)
                    System.out.println(">, ");
                else
                    System.out.print(">");
                tm.put(fs.members.get(j).name, fs.members.get(j).type); // function name, type을 type map에 저장
            }
            TypeMap temp = new TypeMap();
            temp.putAll(typing(fs.members.get(i).locals));// type map에 local variable 저장
            temp.putAll(typing(fs.members.get(i).params));// type map에 parameter 저장
            temp.putAll(globals); // type map에 global variable 저장
            Iterator<Variable> iter = temp.keySet().iterator();
            for (int k = temp.size() - 1; k >= 0; k--) {
                ...
                Type t = temp.get(v);
                System.out.print("<" + v + ", " + t); // local variable, parameter, global variable 출력
                if (k != 0)
                    System.out.println(">, ");
                else
                    System.out.print(">");
            ...
            tm.putAll(temp);
            V(fs.members.get(i).body, tm); // body의 type check
            System.out.println(" }\n");
        }
    }
    ...
    public static void V(Expression e, TypeMap tm) {
        ...
        if (e instanceof Call) {
            Call c = (Call) e;
            check(tm.containsKey(c.name), "undefined function: " + c.name); // type map에 function 정보가 있는지 check
            return;
        }
        throw new IllegalArgumentException("should never reach here");
    }
    ...
    public static void V(Statement s, TypeMap tm) {
        ...
        if (s instanceof Call) {
            Call f = (Call) s;
            check(tm.containsKey(f.name), "undefined function: " + f.name); // type map에 function 정보가 있는지 check
            return;
        }
        if (s instanceof Return) {
            Return r = (Return) s;
            V(r.name, tm); // return variable의 name check
            V(r.result, tm); // return variable의 value check
            return;
        }
        throw new IllegalArgumentException("should never reach here");
    }
