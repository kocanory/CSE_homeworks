...
public class TypeTransformer_edited {
    ...
    public static Expression T(Expression e, TypeMap tm) {
        ...
        if (e instanceof Binary) {
            Binary b = (Binary) e;
            Type typ1 = StaticTypeCheck_edited.typeOf(b.term1, tm); // term1의 type 값 저장
            Type typ2 = StaticTypeCheck_edited.typeOf(b.term2, tm); // term2의 type 값 저장
        ...
        if (e instanceof Unary) {
            Unary u = (Unary) e;
            Type typ = StaticTypeCheck_edited.typeOf(u.term, tm); // term의 type 값 저장
    ...
    public static Statement T(Statement s, TypeMap tm) {
        ...
        if (s instanceof Assignment) {
            ...
            Type srctype = StaticTypeCheck_edited.typeOf(a.source, tm); // source의 type 저장
            ...
            StaticTypeCheck_edited.check(ttype == srctype, "bug in assignment to " + target);
    ...
    public static void main(String args[]) {
        ...
        TypeMap map = StaticTypeCheck_edited.typing(prog.decpart);
        map.display(); // declarations에서 정의된 <변수, 변수형> 정보를 출력
        StaticTypeCheck_edited.V(prog); // 프로그램이 올바른지 확인
        if (StaticTypeCheck_edited.flag == false) { // 만약 program의 type check에서 오류가 발생하지 않았다면 출력
